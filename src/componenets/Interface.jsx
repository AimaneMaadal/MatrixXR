  ...
  return (
    <>
      <ThemeProvider theme={theme}>
        ...
        <div
          className="drawer"
          style={{ width: open ? "auto" : "0", padding: open ? "2rem" : "0" }}
        >
              ...
              <Stack direction="row" spacing={1}>
                <Chip
                  ...
                  onClick={() =>
                    setCategory(
                      category.includes("chair")
                        ? category.filter((category) => category !== "chair")
                        : [...category, "chair"]
                    )
                  }
                />
                <Chip
                  label="Couch"
                  {...(category.includes("couch")
                    ? { color: "primary" }
                    : { variant: "outlined" })}
                  onClick={() =>
                    setCategory(
                      category.includes("couch")
                        ? category.filter((category) => category !== "couch")
                        : [...category, "couch"]
                    )
                  }
                />
                ...
                {brands.map((brandName) => (
                  <Chip
                    ...
                    onClick={() =>
                      setBrand(
                        brand.includes(brandName)
                          ? brand.filter((brand) => brand !== brandName)
                          : [...brand, brandName]
                      )
                    }
                  />
                  ...
                  >
                    <Stack direction="row" alignItems="center">
                      <Rating
                        name="read-only"
                        value={5}
                        sx={{ "& .MuiRating-iconFilled": { color: "#242424" } }}
                        readOnly
                      />
                      &nbsp; &up
                    </Stack>
        ...
        {selectedWalk && selectedWalk.length > 3 ? (
          <div className="selected">
            ...
            <Typography variant="h4">{selectedWalk[8]}</Typography>
            <Chip
              label={"$ " + selectedWalk[7]}
              variant="contained"
              color="primary"
              sx={{ width: 100, height: 30, borderRadius: 5, mb: 2 }}
            />
